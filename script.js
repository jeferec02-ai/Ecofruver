...
// =========================================================
// === FUNCIÓN DE ANIMACIÓN (NUEVA) ========================
// =========================================================

function createFlyAnimation(buttonElement) {
    // 1. Obtener la posición del botón de "Agregar"
    const startRect = buttonElement.getBoundingClientRect();
    
    // 2. Obtener la posición del icono del carrito (el objetivo)
    const cartIcon = document.querySelector('.cart');
    const endRect = cartIcon.getBoundingClientRect();

    // 3. Crear el elemento 'volador'
...
    // 5. Aplicar la transición (vuelo) después de un pequeño retraso
    setTimeout(() => {
        // Mueve el elemento a la posición del carrito
        flyer.style.transform = `translate(${endRect.left - startRect.left}px, ${endRect.top - startRect.top}px) scale(0.2)`;
        flyer.style.opacity = '0';
    }, 50);

    // 6. Eliminar el elemento al finalizar la animación
    setTimeout(() => {
        flyer.remove();
    }, 800); 
}
...
// =========================================================
// === RENDERIZACIÓN Y FILTROS =============================
// =========================================================

// === Renderizar productos (CATÁLOGO PRINCIPAL) ===
...
                    <button class="btn-add" onclick="addToCart(${product.id}, document.getElementById('qty-${product.id}').value, this)" ${disabledBtn}>Agregar</button>
        </div>
...
// === Añadir al carrito (con cantidad y animación) ===
function addToCart(id, quantityInput, buttonElement) {
    const product = products.find(p => p.id === id);
    // Aseguramos que quantity sea un número válido y al menos 1
...
            // Llamar a la animación solo si se agregó con éxito
            if (buttonElement) { 
                createFlyAnimation(buttonElement); 
            }
        } else {
            alert(`No puedes agregar esa cantidad. Solo quedan ${product.stock - cartItem.quantity} ${product.unit} disponibles.`);
            return;
        }
    } else {
...
            }
        } else {
            alert(`No hay suficiente stock. Solo quedan ${product.stock} ${product.unit} disponibles.`);
            return;
        }
    }
...
                    <button class="btn-add" onclick="addToCart(${product.id}, 1, this)">Agregar</button>
        </div>
      </div>
    `;
...
                    <button class="btn-add" onclick="addComboToCart(${combo.id}, this)">Agregar Combo</button>
        </div>
      </div>
    `;
...
// === Añadir Combo al carrito (con animación) ===
function addComboToCart(comboId, buttonElement) {
